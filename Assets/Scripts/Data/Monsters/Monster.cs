...
{
    public string name; //name of the monster that is displayed in-game.
    
    public Type type1; //first type. Always fill.
    public Type type2; //secondary type. Use Type.NONE if none. NEVER LEAVE AS 'NULL', IT WILL BREAK THINGS
    ...
    public GameObject model; //Model that the monster will render. This is a Unity Prefab. Read the comment in the Constructor for more info.

    public Monster(string n, Type t1, Type t2, int hp, int atk, int def, int spatk, int spdef, int spd, Move m1, Move m2, Move m3, Move m4, GameObject mod){
        name = n;
        type1 = t1;
    ...
    ///<summary>
    /// This constructor uses an instance of the MonsterTemplate scriptable to create the full monster data structure, which is used in actual gameplay.
    ///</summary>
    public Monster(MonsterTemplate mon) : this(mon.name, mon.type1, Type.NONE, mon.HP, mon.ATK, mon.DEF, mon.spATK, mon.spDEF, mon.SPEED, mon.move1, mon.move2, mon.move3, mon.move4, mon.prefab) {}

    /* ---DEPRECATED--- this constructor makes it possible to "clone" the static monsters from the monster list into non-static instances. This is very important for gameplay. (not anymore)
    ...
    my dumb a** forgot that since the monster values are static, they all point to the same data.
